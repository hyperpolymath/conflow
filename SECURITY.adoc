...
. Email security concerns to: `+security@conflow.dev+` (or create a
confidential issue)
...
conflow requires: - Read access to configuration files - Write access to
output directories and cache - Execute access for CUE and Nickel
binaries
...
`+shell+` tool type
* Users should review `+.conflow.yaml+` files from untrusted sources
before running
